...
import { getAllUsers, getUserBySkill, getUserByName } from '../user/helper/userapicalls';
...
function FindDevelopers() {
       ...
       const handleSearch = (event) => {
              ...
              setIsLoading(true);
              if(isSkill) {
                     getUserBySkill({skill: search}, authtoken)
                            .then(data => {
                                   if(data.error)
                                          setError(data.error)
                                   else {
                     ...
                     getUserByName({name: search}, authtoken)
                            .then(data => {
                                   if(data.error)
                                          setError(data.error)
                                   else {
                     ...
                     <Header find={true} />
                            <div className="container">
                                          ...
                                          <div className="col-10 mx-auto">
                                                 <div className="input-group mb-3">
                                                        <div className="input-group-prepend">
                                                               <select className="form-select text-white theme-bg" defaultValue={"name"} onChange={(e) => e.target.value=="name" ? setIsSkill(false) : setIsSkill(true)} aria-label="Default select example">
                                   ...
                                   <div className="row mb-5">
                                          {devs.map((dev, index) => {
                                                 return <UserCard user={dev} type={0} key={index} />
                                          })}
                                          {devs.length==0 && (<div className="col-8 bg-white mx-auto text-center p-5">
                                                 <h3><strong>No developers found!</strong></h3>
                                          </div>)}
                                   </div>
                            </div>
                     <Footer />
