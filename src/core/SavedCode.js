...
import { getCode, executeCode, fetchResult, saveCode, updateCode, deleteCode } from './helper/coreapicalls';
import MonacoEditor from '@uiw/react-monacoeditor';

function SavedCode(props) {
       ...
       const [codeLink, setCodeLink] = React.useState(`${process.env.REACT_APP_FRONTEND}compiler/code/`+codeId+"/");
              ...
              <div>
                     <Header tools={true} />
                            <div className="container mt-5">
                                   ...
                                   <h3>Code: </h3>
                                   {/* <div>
                                          <pre><textarea onChange={(e) => {setCode(e.target.value); setError(false)}} value={code} className="form-control mt-3" spellCheck="false" id="floatingInput" placeholder="Start coding..." rows="5" style={{height:"450px"}} onKeyDown={useTab}></textarea></pre>
                                   </div> */}
                                   <div>
                                          <pre><MonacoEditor
                                                 language={language}
                                   ...
                                   <h3>Output: </h3>
                                   <div>
                                          <pre><textarea value={output} className="form-control mt-3 mb-5 unique-color-dark text-white" id="floatingInput" placeholder="" rows="5" style={{height:"250px"}} disabled></textarea></pre>
                                   </div>
                                   
                                   {!userId=="" && userId==user._id && (<><hr />
                                          {!codeLink=="" &&(<div className="row">
                                                 ...
                                                 <div className="col-2 mx-auto mb-5">
                                                        <button onClick={handleCopy} style={{borderRadius:"10px"}} className="btn btn-purple rounded btn-login text-uppercase fw-bold" type="submit"><i className="fas fa-copy"></i>  Copy Link</button>
                                                 </div>
                                          </div>)}
                                          {codeLink=="" && (<div className="mx-auto d-grid mb-5">
                                                 <button onClick={onClickSave} style={{borderRadius:"10px"}} className="btn btn-purple rounded btn-login text-uppercase fw-bold" type="submit">{isGenerating ? <span className="spinner-border spinner-border-sm text-white"></span> : <i className="fas fa-share-alt"></i>}{isGenerating ? "  Generating..." : "  Generate sharable link"}</button>
                                          </div>)}
                                          {!codeLink=="" && (<div className="mx-auto d-grid mb-5">
                                                 <button onClick={onClickSaveChanges} style={{borderRadius:"10px"}} className="btn btn-purple rounded btn-login text-uppercase fw-bold" type="submit">{isGenerating ? <span className="spinner-border spinner-border-sm text-white"></span> : <i className="fas fa-save"></i>}{isChangesSaved ? "  Changes Saved" : (isGenerating ? "  Saving..." : "  Save changes")}</button>
                                          </div>)}
                                          {!codeLink=="" && (<div className="mx-auto d-grid mb-5">
                                                 <button data-bs-toggle="modal" data-bs-target="#deleteCodePop" style={{borderRadius:"10px"}} className="btn btn-danger rounded btn-login text-uppercase fw-bold" type="submit">{isDeleting ? <span className="spinner-border spinner-border-sm text-white"></span> : <i className="fas fa-trash-alt"></i>}{isDeleting ? "  Deleting..." : "  Delete Code"}</button>
                                          </div>)}
                                   </>)}
                            </div>
                     <Footer />
